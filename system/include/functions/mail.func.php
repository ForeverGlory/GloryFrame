...
function runlog($mode = 'SMTP',$b = '',$c = '',$d='') {
    error_log('<?php exit();?>' . date('m-d H:i:s', SYS_TIME) . ' | ' . $mode . ' | ' . $b . ' | ' . $c . ' | ' . $d . "\r\n", 3, CACHE_PATH . 'error' . DIRECTORY_SEPARATOR . 'errormail_log_' . date("Y_m_d", time()) . '.php');
}
...
function sendmail($toemail, $subject, $message, $cfg = array()) {
	$adminemail = 'foreverglory@qq.com';
	if($cfg && is_array($cfg)) {
		$adminemail = $cfg['from'];
		$mail = $cfg;
		$mail_type = $cfg['mail_type']; //邮件发送模式
	} else {
		$cfg = ys_base::load_config('email');
	...
	//发信者
    $email_from = preg_match('/^(.+?) \<(.+?)\>$/',$mail['from'], $from) ? '=?'.CHARSET.'?B?'.base64_encode($from[1])."?= <$from[2]>" : $mail['from'];
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: ForeverGlory {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=".CHARSET."{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
	...
	fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." ForeverGlory\r\n");
	$lastmessage = fgets($fp, 512);
