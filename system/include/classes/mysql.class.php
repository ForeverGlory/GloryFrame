...
final class mysql
{
    ...
    public function select($data, $table, $where = '', $limit = '', $order = '', $group = '', $key = '')
    {
        ...
        {
            $table[$n] = "`" . $this->config['database'] . "`.`" . $v . "`";
        }
        $table = implode(",", $table);
     ...
     * @return boolean
     */
    public function insert($data, $table, $return_insert_id = true, $replace = false)
    {
    ...
    {
        $this->errormsg = "<b>MySQL Query : </b> $sql <br /><b> MySQL Error : </b>" . $this->error() . " <br /> <b>MySQL Errno : </b>" . $this->errno() . " <br /><b> Message : </b> $message <br /><a href='http://faq.mysql.cn/?errno=" .
            $this->errno() . "&msg=" . urlencode($this->error()) . "' target='_blank' style='color:red'>Need Help?</a>";
        $msg = $this->errormsg;
        if (ys_base::load_config("system", "errormysql"))
        {
        ...
        else
        {
            ...
            error_log('<?php exit();?>' . SYS_DATE . ' | ' . $msg . "\r\n", 3, CACHE_PATH . 'error' . DIRECTORY_SEPARATOR . 'errormysql_log_' . date("Y_m_d", time()) . '.php');
            exit;
        }
    }
